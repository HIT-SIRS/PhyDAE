...
def run_test(opts, net, dataset, factor=8):
    ...
    if opts.save_results:
        pathlib.Path(os.path.join(opts.save_dir, opts.benchmarks[0])).mkdir(parents=True,
                                                                                                exist_ok=True)
    ...
    with torch.no_grad():
        for ([clean_name, de_id], degrad_patch, clean_patch) in tqdm(testloader):
            ...
            if opts.save_results:
                ...
                save_path = os.path.join(opts.save_dir, opts.benchmarks[0], save_name)
...
# test Dedark
def run_dedark(opts, net, dataset, factor=8):
    run_test(opts, net, dataset, factor)
...
def main(opt):
    ...
    # Load model
    assert os.path.exists(opt.ckpt_path), f"Checkpoint not found: {opt.ckpt_path}"
    net = PLTestModel.load_from_checkpoint(opt.ckpt_path, opt=opt).cuda()
    net.eval()
